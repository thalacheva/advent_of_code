...
$min = 534
def move(map, x, y, minute)
  n = map.length
  m = map[0].length

  return if n - 2 - x + m - 2 - y + minute >= $min
  ...
  if x < n - 2 && map[x+1][y].length == 1 && map[x+1][y][0] == '.'
    move(copy(map), x + 1, y, minute + 1)
  end

  if y < m - 2 && map[x][y+1].length == 1 && map[x][y+1][0] == '.'
    move(copy(map), x, y + 1, minute + 1)
  end

  if map[x][y].length == 1
    move(copy(map), x, y, minute + 1)
  end
